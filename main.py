...
feature_columns = ['anchor_ratio',
                   'trans_range(m)', 'node_density', 'iterations']
...
def find_optimal_clusters(data, max_clusters=8):
    """Find optimal number of clusters using Fuzzy Partition Coefficient"""
...
for i in range(optimal_clusters):
    cluster_centers_original[i, :- 1]\
        = scaler_X.inverse_transform(cntr[i, :-1].reshape(1, -1))[0]
    cluster_centers_original[i, - 1]\
        = scaler_y.inverse_transform([[cntr[i, -1]]])[0][0]
...
def create_membership_functions(variable_name, data_column, cluster_centers_for_var, n_clusters):
    ...
    if n_clusters == 2:
        ...
        for i, center in enumerate(centers):
            if i == 0:
                left = min_val
                right = (centers[i] + centers[i+1]) / \
                    2 if i < len(centers) - 1 else max_val
...
for i, feature in enumerate(feature_columns):
    ...
    fuzzy_var = ctrl.Antecedent(universe, feature.replace(
        '(m)', '').replace('trans_range', 'trans_range'))
    ...
    for mf_name, params in mf_params.items():
        print(
            f"  {mf_name}: trimf([{params[0]:.2f}, {params[1]:.2f}, {params[2]:.2f}])")
    ...
    for i in range(n_clusters):
        ...
        for j, feature in enumerate(feature_names):
            ...
            for mf_name, mf_params in feature_mf_params.items():
                ...
                membership_val = fuzz.trimf(
                    np.array([feature_value]), mf_params)[0]
...
def evaluate_fuzzy_system(simulation, X_test, y_true, feature_names):
    ...
    for i, sample in enumerate(X_test):
        try:
            # Set input values
            for j, feature in enumerate(feature_names):
                clean_feature = feature.replace('(m)', '').replace(
...
print(
    f"  \
Failed predictions: {failed_count}/{len(X)} ({failed_count/len(X)*100:.1f}%)")
...
# Performance summary
ax4.text(0.1, 0.8, 'Performance Metrics:', fontsize=14,
...
ax4.text(0.1, 0.4, f'Failed: {failed_count}/{len(X)}',
         fontsize=12, transform=ax4.transAxes)
ax4.text(0.1, 0.2, f'Clusters: {optimal_clusters}',
         fontsize=12, transform=ax4.transAxes)
ax4.text(0.1, 0.1, f'Rules: {len(extracted_rules)}',
         fontsize=12, transform=ax4.transAxes)
ax4.set_xlim(0, 1)
ax4.set_ylim(0, 1)
ax4.axis('off')
...
print(
    "Save results with: \
            results_df.to_csv('fuzzy_system_results.csv', index=False)")
